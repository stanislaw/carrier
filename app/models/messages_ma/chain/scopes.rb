  ...
  class Chain 
    module Scopes
      def self.extended(base)
        ...
        base.scope :with_messages_sent, lambda{|user| base.where("\"participants\" ~ '^---\\s\\n-\\D#{user.id}\\D'").reversed }
      ...
      def with_messages_sent_by(user, limit = nil)
        with_messages_sent(user)
      end
